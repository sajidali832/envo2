...
const NavLink = ({ href, icon: Icon, label }: { href: string; icon: LucideIcon; label: string }) => {
    ...
    return (
        <Link href={href} className={cn(
            "flex flex-col items-center justify-center h-full flex-1 gap-1 p-2 transition-colors duration-200 rounded-md",
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden">
            <div className="bg-card/95 backdrop-blur-sm border-t p-2">
                <nav className="flex items-center justify-around h-full gap-2">
                   {navItems.map((item) => (
